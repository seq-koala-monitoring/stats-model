...
nimble_sat_model <- nimbleCode({
	...
	if (SpCAR == 1) {
		# correlated spatial random-effects for each grid
		sd[1:NLGrids] ~ dcar_normal(adj = AdjS[1:NLGridAdjs], weights = WeightsAdjS[1:NLGridAdjs], num = NumAdjS[1:NLGrids], tau = tau_sd, zero_mean = 1)
	} else {
		# independent spatial random-effects for each small grid
		for (i in 1:NSGrids) {
			sd[i] <- 0 #dnorm(0, sd = sigma_sd)
		}
	}
	...
	if (VarTrend == 1) {
		...
		if (Order == 2) {
			# time-step temporally structured random-effect for densities (based on second order intrisic CAR model) and independent among genetic populations
			# note here weights are taken from Breslow & Clayton (1993) and mean is constrained to be zero
			# annual time step
			for (i in 1:NGPops){
    			std[i, 1:NTime2] ~ dcar_normal(adj = AdjT2[1:NTimeAdjs2], weights = WeightsAdjT2[1:NTimeAdjs2], num = NumAdjT2[1:NTime2], tau = tau_std, c = 2)
  			}
		} else {
			# time-step temporally structured random-effect for densities (based on first order intrisic CAR model) and independent among genetic populations
	...
	for (i in 1:NSGrids) {
		if (SpCAR == 1) {
			# linear predictor for time-constant predictors, including spatial random-effect
			a[i] <- inprod(beta_d[1:NX], X[i, 1:NX]) + sd[i]
		} else {
			# linear predictor for time-constant predictors, including spatial random-effect
			a[i] <- inprod(beta_d[1:NX], X[i, 1:NX]) + sd[i]
		}
	...
	# mean expected density for each time step
	for (t in 1:(LastDateID - FirstDateID + 1)) {
	...
	for (i in 1:(NX + NY)) {
		beta_d[i] ~ dnorm(0, sd = 10)
	}
	for (i in 1:NZ) {
		beta_shn[i] ~ dnorm(0, sd = 10)
	}
	nbr ~ dunif(0,50)
})
...
nimble_sel_model <- nimbleCode({

	# process model

	# variable selection for koala density
	for(i in 1:(NX + NY)) {
	...
	for (i in 1:NSGrids) {
		if (SpCAR == 1) {
			# linear predictor for time-constant predictors, including spatial random-effect
			a[i] <- inprod(zbeta_d[1:NX], X[i, 1:NX]) + sd[LGridID[i]]
		} else {
			# linear predictor for time-constant predictors, including spatial random-effect
			a[i] <- inprod(zbeta_d[1:NX], X[i, 1:NX]) + sd[GenPopID[i]]
		}
		...
		for (t in 1:(LastDateID - FirstDateID + 1)) {
			...
			# and temporal random effect
			if (VarTrend == 1) {
				log(d[i, t]) <- a[i] + std[GenPopID[i], floor((t - 1) / 2) + 1] + inprod(zbeta_d[(NX + 1):(NX + NY)], Y[i, FirstDateID - 1 + t - Lag, 1:NY])
			} else {
				log(d[i, t]) <- a[i] + td[floor((t - 1) / 2) + 1] + inprod(zbeta_d[(NX + 1):(NX + NY)], Y[i, FirstDateID - 1 + t - Lag, 1:NY])
			}
		}
	}
