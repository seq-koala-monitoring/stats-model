...
# nimble code for exponential population growth model

NimbleCodeEXP <- nimbleCode({
	...
	# large grid spatially structured random-effect for initial densities (based on first order intrisic CAR model)
	# note here weights are taken to be 1
	sdi[1:NLGrids] ~ dcar_normal(adj = AdjS[1:NLGridAdjs], weights = WeightsAdjS[1:NLGridAdjs], num = NumAdjS[1:NLGrids], tau = tau_sdi)

	# large grid spatially structured random-effect for growth rate (based on first order intrisic CAR model)
	# note here weights are taken to be 1
	sl[1:NLGrids] ~ dcar_normal(adj = AdjS[1:NLGridAdjs], weights = WeightsAdjS[1:NLGridAdjs], num = NumAdjS[1:NLGrids], tau = tau_sl)

	# time-step temporally structured random-effect for lambda (based on first order intrisic CAR model)
	# note here weights are taken to be 1 and mean is constrained to be zero
	tl[1:NTime] ~ dcar_normal(adj = AdjT[1:NTimeAdjs], weights = WeightsAdjT[1:NTimeAdjs], num = NumAdjT[1:NTime], tau = tau_tl, zero_mean = 1)

	# time-step unstructured random-effect for growth rate
	#for (i in 1:(LastDateID - FirstDateID)) {
	#	tl[i] ~ dnorm(mean = 0, sd = sigma_tl)
	#}
	...
	for (i in 1:NSGrids) {
		...
		for (t in 2:(LastDateID - FirstDateID + 1)) {
			# predictor for density including time-dependent variables, unstructured (for time step) stochasticity
			# and spatially structured (at the large grid scale) stochasticity
			mu_l[i, t - 1] <- exp(sl[LGridID[i]] + tl[t - 1] + inprod(beta_l[1:NY], Y[i, FirstDateID - 1 + t - 1 - Lag, 1:NY]))
	...
	# mean spatial effects
	mean_sdi <- mean(sdi[1:NLGrids])
	mean_sl <- mean(sl[1:NLGrids])
	...
	# mean lambda
	for (t in 1:(LastDateID - FirstDateID)) {
		mean_l[t] <- mean(lambda[1:NSGrids, t])
	}
