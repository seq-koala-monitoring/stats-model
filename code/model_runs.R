...
for (Order in 1:1) {
  for (Lag in 0:2) {
    for (VarTrend in 0:1) {
      ...
      # fit model
      if (VarTrend == 1) {
        Samples <- parLapply(cl = cl, X = 1:3, fun = fit_sel_model, Seeds = c(seed, seed + 20, seed + 40), Iter = 15000, Burnin = 5000, Thin = 1, Monitors = c("beta_d", "beta_shn", "sigma_sd", "sd", "sigma_std", "std", "nbrst", "nbraa", "nbrli", "o", "sigma_o"), Calculate = FALSE, EnableWAIC = TRUE, Data = FitData, Code = nimble_sat_model)
      } else {
        Samples <- parLapply(cl = cl, X = 1:3, fun = fit_sel_model, Seeds = c(seed, seed + 20, seed + 40), Iter = 15000, Burnin = 5000, Thin = 1, Monitors = c("beta_d", "beta_shn", "sigma_sd", "sd", "sigma_td", "td", "nbrst", "nbraa", "nbrli", "o", "sigma_o"), Calculate = FALSE, EnableWAIC = TRUE, Data = FitData, Code = nimble_sat_model)
      }
